...
fn main() {
    ...
    let mut email_acc= EmailAccount::new(clargs.mail.unwrap(), clargs.smtp_srvr.unwrap(), clargs.imap_srvr.unwrap(), clargs.username.clone().unwrap(),clargs.pass.unwrap(), clargs.smtp_port.unwrap(), clargs.imap_port.unwrap());
